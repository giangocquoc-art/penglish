...
import { PooSystemState } from '../p-english/PooSystemState';
...
export function LessonFlashcardPractice({ lesson, onWhaleMoodChange }: { lesson: EnglishLesson; onWhaleMoodChange?: (mood: WhaleMood) => void }) {
  ...
  const [meaningPenalizedIds, setMeaningPenalizedIds] = useState<string[]>([]);
  ...
  const guardedFlip = () => {
    if (!meaningVerified && !meaningRevealed) return;
    setIsFlipped((value) => !value);
  };

  const checkMeaning = () => {
    if (!currentCard || meaningVerified || meaningRevealed) return;
    ...
    const alreadyPenalized = meaningPenalizedIds.includes(currentCard.id);
    if (!alreadyPenalized) {
      updateWordAfterWrongAnswer(vocabularyWordIdFromFlashcard(currentCard.id));
    }
    const hearts = alreadyPenalized ? undefined : loseHeart('flashcard-meaning');
    setMeaningPenalizedIds((current) => current.includes(currentCard.id) ? current : [...current, currentCard.id]);
    const heartsText = hearts ? 'Bạn mất 1 bọt biển. ' : '';
    setMeaningFeedback({ tone: 'red', text: `Chưa đúng. Đáp án là: ${currentCard.back}. ${heartsText}Cá voi vẫn bơi cùng bạn — bấm “Lật thẻ” để xem đáp án nhé.`, hearts });
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.code === 'Space') {
        ...
        if (meaningVerified || meaningRevealed) setIsFlipped((value) => !value);
        return;
      }
      if (!isFlipped) return;
  ...
  }, [showSummary, meaningVerified, meaningRevealed, isFlipped, currentCard, markNeedsReview, markRemembered]);
  ...
  if (allCards.length === 0) {
    return (
      <Box bg={COLORS.bg} minH="calc(100vh - 72px)" px="6" py="8">
        <Box maxW="760px" mx="auto" bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="3xl">
          <PooSystemState variant="empty" title="Poo chưa có thẻ từ cho bài này" description="Bạn có thể mở sổ từ vựng để chọn một nhóm từ đang sẵn sàng." actionLabel="Mở sổ từ vựng" actionTo="/vocabularies" />
        </Box>
      </Box>
    );
  }
  ...
  if (isDueReview && dueFlashcards.length === 0) {
    return (
      <Box bg={COLORS.bg} minH="calc(100vh - 72px)" px="6" py="8">
        <Box maxW="760px" mx="auto" bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="3xl">
          <PooSystemState variant="empty" title="Hôm nay chưa có thẻ đến hạn ôn" description="Các thẻ đã nhớ sẽ quay lại đúng lịch. Nếu muốn, bạn vẫn có thể luyện toàn bộ thẻ ngay bây giờ." actionLabel="Luyện toàn bộ thẻ từ" actionTo={`/practice?lessonId=${lesson.id}&mode=flashcard`} />
        </Box>
      </Box>
    );
  }
        ...
        <VStack align="stretch" gap="5">
          <Box bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="3xl" p={{ base: '5', md: '7' }}>
            ...
            <Text mt="3" color={COLORS.muted} fontSize="sm">
              {isDueReview ? 'Poo đang nhắc các thẻ đã đến lúc ôn lại cho bạn.' : 'Đây là lượt từ vựng: đoán nghĩa từ vựng, nghe mẫu, rồi đọc to ví dụ.'}
            </Text>
          </Box>
          ...
          {showSummary ? (
            ...
            <>
                  ...
                  <VStack gap="5" w="100%">
                    ...
                    <Box w="100%" maxW="820px" onClick={(event) => event.stopPropagation()}>
                      <AnimatedWordScene
                        ...
                        meaningVi={meaningVerified || meaningRevealed ? currentCard.back : 'Nhập nghĩa tiếng Việt để mở khóa'}
                        ...
                        showSpeechBubble={meaningVerified || meaningRevealed}
                      />
                    </Box>
                    <Text color={COLORS.muted}>{meaningVerified ? 'Bạn đã nhập đúng nghĩa. Bấm thẻ, nhấn Space, hoặc dùng nút “Lật thẻ”.' : meaningRevealed ? 'Đáp án đã hiện ở trên. Bấm thẻ, nhấn Space, hoặc dùng nút “Lật thẻ” để xem mặt sau.' : 'Nhập đúng nghĩa tiếng Việt bên dưới để mở khóa mặt sau.'}</Text>
                  </VStack>
                ) : (
                ...
                <Box bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="2xl" p="5">
                  ...
                  <HStack mt="3" gap="3" align="start" flexDirection={{ base: 'column', md: 'row' }}>
                    <Input
                      ...
                      isDisabled={meaningVerified || meaningRevealed}
                    />
                    <Button onClick={checkMeaning} isDisabled={!meaningInput.trim() || meaningVerified || meaningRevealed} borderRadius="full" bg={COLORS.green} color="white" flexShrink={0}>
                      Poo xem nghĩa
                    </Button>
                  </HStack>
                  {meaningFeedback ? (
              ...
              <Flex justify="center" gap="3" wrap="wrap">
                ...
                <Button onClick={guardedFlip} isDisabled={!meaningVerified && !meaningRevealed} borderRadius="full" bg={COLORS.primary} color="white">
                  Lật thẻ
                </Button>
                {isFlipped ? (
                  <>
...
function Summary({
  ...
  return (
    <Box bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="3xl" p={{ base: '6', md: '8' }}>
      <Text fontSize={{ base: '2xl', md: '3xl' }} fontWeight="950" color={COLORS.text}>Tổng kết lượt từ vựng</Text>
