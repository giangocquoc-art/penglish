...
import { getVocabularyStats } from '../lib/p-english/vocabulary-review';
import { getCompletedLessons } from '../lib/p-english/local-progress';
...
export function ProfilePage() {
  ...
  useEffect(() => {
    function readCoin() {
      try {
        ...
        setCoinCount(typeof u?.coin === 'number' ? u.coin : 0);
      } catch {
        setCoinCount(0);
      }
    }
    ...
    readCoin();
    const onStorage = (e: StorageEvent) => { if (e.key === 'currentUser') readCoin(); };
  ...
  // Local data for stat tiles — updates when the underlying stores change.
  const vocStats = getVocabularyStats();
  ...
  const stats: Stat[] = [
    ...
    { label: 'Xu', value: coinCount, tint: '#B7791F', bg: PROFILE_COLORS.rewardYellow, icon: Trophy },
    { label: 'Từ đã thuộc', value: vocStats.known, tint: PROFILE_COLORS.successGreen, bg: '#EAFBF0', icon: BookOpen },
    { label: 'Bộ học', value: completedLessonIds.length, tint: PROFILE_COLORS.deepBlue, bg: PROFILE_COLORS.softBlue, icon: BookOpen },
  ];
        ...
        <Flex direction={{ base: 'column', md: 'row' }} gap="6" align={{ base: 'center', md: 'start' }}>
          ...
          <Box flex="1" textAlign={{ base: 'center', md: 'left' }} minW="0">
            <HStack gap="2" mb="1" justify={{ base: 'center', md: 'start' }} wrap="wrap">
              <Text as="h1" fontSize={{ base: '2xl', md: '3xl' }} fontWeight="900" color={PROFILE_COLORS.text}>{displayName}</Text>
            ...
            <HStack gap="2" justify={{ base: 'center', md: 'start' }} wrap="wrap">
              {session.isSignedIn ? (
                <>
                  <Button size="sm" minH={{ base: '44px', md: '32px' }} bg={PROFILE_COLORS.oceanBlue} color="white" borderRadius="full" leftIcon={<Icon as={RefreshCw} />} isLoading={syncing} onClick={syncProgress} _hover={{ bg: PROFILE_COLORS.deepBlue }}>
                    Lưu tiến độ lên tài khoản
                  </Button>
                  <Button size="sm" minH={{ base: '44px', md: '32px' }} variant="outline" borderRadius="full" leftIcon={<Icon as={LogOut} />} onClick={() => void auth.signOut()}>
                    Đăng xuất
                  </Button>
                </>
              ) : (
                <Button size="sm" minH={{ base: '44px', md: '32px' }} bg={PROFILE_COLORS.oceanBlue} color="white" borderRadius="full" onClick={() => void auth.signInWithGoogle()} _hover={{ bg: PROFILE_COLORS.deepBlue }}>
