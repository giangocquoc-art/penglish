...
// Listening-sentence length budget grows with proficiency. A1 learners need short,
// airy sentences; higher levels are expected to process longer natural chunks.
function maxListeningWordCount(lesson: EnglishLesson): number | null {
  if (lesson.level === 'Beginner / A1') return 14;
  if (lesson.level === 'Elementary / A2') return 20;
  // B1 and up: no hard length ceiling — long listening text is legitimate.
  return null;
}

function randomizationWarningSeverity(message: string): LessonContentValidationWarning['severity'] {
  if (/missing a correct answer|cannot be built/i.test(message)) return 'error';
  if (/duplicate normalized tokens/i.test(message)) return 'info';
  return 'warning';
}
...
export function validateLessonContent(lesson: EnglishLesson): LessonContentValidationWarning[] {
  ...
  checkDuplicateIds(warnings, lessonId, 'dialogue', lesson.miniDialogues.map((item) => item.id));
  ...
  if (lesson.id.startsWith('reading-')) {
    const uniqueSentenceChunks = new Set(
      lesson.vocabulary
        .filter((item) => item.partOfSpeechOrType === 'sentence chunk')
        .map((item) => normalizeText(item.term)),
    );
    if (uniqueSentenceChunks.size < 3) {
      pushWarning(warnings, lessonId, 'warning', 'vocabulary', 'Generated reading lessons should expose at least 3 distinct sentence chunks for warm-up and retelling.');
    }
  }
  ...
  if (lesson.vocabulary.length > 0 && lesson.flashcards.length !== lesson.vocabulary.length) {
    pushWarning(
      warnings,
      lessonId,
      'error',
      'flashcards',
      `Vocabulary-first lessons should keep flashcard count (${lesson.flashcards.length}) aligned with vocabulary count (${lesson.vocabulary.length}).`,
    );
  }
  ...
  lesson.listeningPractice.forEach((item) => {
    ...
    const listeningLimit = maxListeningWordCount(lesson);
    if (listeningLimit !== null && wordCount > listeningLimit) {
      pushWarning(warnings, lessonId, 'warning', 'listening', `Listening text (${wordCount} words) exceeds the ${lesson.level} budget of ${listeningLimit} words.`, item.id);
    }
  });

  const listeningTextCounts = lesson.listeningPractice.reduce<Map<string, number>>((counts, item) => {
    const key = normalizeText(item.text);
    counts.set(key, (counts.get(key) ?? 0) + 1);
    return counts;
  }, new Map());
  if ([...listeningTextCounts.values()].some((count) => count > 1)) {
    pushWarning(warnings, lessonId, 'warning', 'listening', 'Listening practice repeats the same sentence in multiple items instead of progressing to a new example.');
  }
  ...
  lesson.miniDialogues.forEach((dialogue) => {
    const uniqueLines = new Set(dialogue.lines.map((line) => normalizeText(line.text)).filter(Boolean));
    if (dialogue.lines.length > 1 && uniqueLines.size === 1) {
      pushWarning(warnings, lessonId, 'warning', 'dialogue', 'Dialogue repeats the same line for every speaker instead of creating a real exchange.', dialogue.id);
    }
  });
  ...
  const totalAssessmentTasks = lesson.quizQuestions.length + lesson.fillBlankTasks.length + lesson.sentenceOrderingTasks.length;
  const acceptedQuizTotals = new Set([lesson.quizQuestions.length, totalAssessmentTasks]);
  if (!acceptedQuizTotals.has(lesson.completionCriteria.totalQuizQuestions)) {
    pushWarning(warnings, lessonId, 'error', 'completionCriteria', 'totalQuizQuestions does not match quizQuestions length or total assessment task count.');
  }
