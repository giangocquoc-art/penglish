...
import { BLOG_POSTS, getBlogPostBySlug, getBlogPostPath, type BlogPost } from '../data/blogPosts';
      ...
      <Container maxW="1120px" px="0">
        <VStack align="stretch" gap={{ base: '5', md: '7' }}>
          <Box className="penglish-glass-card" border="1px solid" borderColor="rgba(186,230,253,0.9)" bg="rgba(255,255,255,0.84)" borderRadius={{ base: '30px', md: '42px' }} p={{ base: '5', md: '9' }} boxShadow="0 26px 80px rgba(31,111,214,0.13)" position="relative" overflow="hidden">
            ...
            <VStack position="relative" align={{ base: 'center', md: 'start' }} textAlign={{ base: 'center', md: 'left' }} gap="4">
              ...
              <Text as="h1" fontSize={{ base: '3xl', md: '5xl' }} fontWeight="950" lineHeight="1.05" color={OCEAN_TOKENS.text} maxW="880px">
                Blog PooEnglish: mẹo học tiếng Anh nhỏ, dễ thương và dùng được mỗi ngày
              </Text>
              <Text fontSize={{ base: 'md', md: 'xl' }} lineHeight="1.85" color={OCEAN_TOKENS.muted} fontWeight="700" maxW="860px">
                Poo gom ở đây những bài viết thân thiện về shadowing, từ vựng, luyện nghe, mất gốc và lộ trình học. Mỗi bài là một chiếc phao nhỏ để bạn bơi tiếp mà không bị ngợp.
              </Text>
              <HStack gap="3" wrap="wrap" justify={{ base: 'center', md: 'start' }}>
                ...
                <Button as={RouterLink} to="/48-ngay-lay-goc" borderRadius="full" bg="white" color={OCEAN_TOKENS.deepBlue} size="lg" border="1px solid" borderColor={OCEAN_TOKENS.borderStrong} _hover={{ bg: OCEAN_TOKENS.softBlue }}>
                  Xem 48 ngày lấy gốc
                </Button>
              </HStack>
            </VStack>
          ...
          <SimpleGrid columns={{ base: 1, md: 2 }} gap="4">
            {BLOG_POSTS.map((post) => (
              <Box key={post.slug} as="article" bg="rgba(255,255,255,0.84)" border="1px solid" borderColor="rgba(186,230,253,0.82)" borderRadius="3xl" p={{ base: '4', md: '6' }} boxShadow="0 14px 34px rgba(31,111,214,0.07)">
                <VStack align="stretch" gap="3" h="100%">
                  ...
                  <Text as="h2" fontSize={{ base: 'xl', md: '2xl' }} fontWeight="950" lineHeight="1.22" color={OCEAN_TOKENS.text}>
                    <RouterLink to={getBlogPostPath(post)}>{post.title}</RouterLink>
                  </Text>
                  <Text color={OCEAN_TOKENS.muted} fontWeight="650" lineHeight="1.8" flex="1">
                    {post.excerpt}
                  </Text>
                  <HStack gap="2" wrap="wrap">
                    {post.keywords.slice(0, 3).map((keyword) => (
                      <Text key={keyword} as="span" px="2.5" py="1" borderRadius="full" bg="rgba(232,244,255,0.86)" color={OCEAN_TOKENS.deepBlue} fontSize="xs" fontWeight="850">
                        {keyword}
                      </Text>
                    ))}
                  </HStack>
            ...
            <VStack position="relative" align="stretch" gap={{ base: '5', md: '6' }}>
              <VStack align={{ base: 'center', md: 'start' }} textAlign={{ base: 'center', md: 'left' }} gap="4">
                ...
                <HStack gap="3" wrap="wrap" justify={{ base: 'center', md: 'start' }}>
                  ...
                  <Button as={RouterLink} to="/blog" borderRadius="full" bg="white" color={OCEAN_TOKENS.deepBlue} size="lg" border="1px solid" borderColor={OCEAN_TOKENS.borderStrong} _hover={{ bg: OCEAN_TOKENS.softBlue }}>
                    Xem thêm bài viết
                  </Button>
                </HStack>
              </VStack>
                  ...
                  <Box key={section.heading} id={`section-${index + 1}`} scrollMarginTop="96px" bg="rgba(255,255,255,0.9)" border="1px solid" borderColor="rgba(186,230,253,0.72)" borderRadius="3xl" p={{ base: '4', md: '6' }}>
                    ...
                    <VStack align="stretch" gap="3">
                      {section.body.map((paragraph) => (
                        <Text key={paragraph.slice(0, 80)} as="p" color={OCEAN_TOKENS.muted} fontWeight="650" lineHeight="1.95">
                          {paragraph}
                        </Text>
                      ))}
                    </VStack>
              ...
              <Box bg="rgba(255,255,255,0.9)" border="1px solid" borderColor="rgba(186,230,253,0.82)" borderRadius="3xl" p={{ base: '4', md: '6' }}>
                <HStack mb="4" color={OCEAN_TOKENS.deepBlue}>
                  <Icon as={Compass} />
                  <Text as="h2" fontSize={{ base: 'xl', md: '2xl' }} fontWeight="900">Bơi tiếp cùng Poo</Text>
                </HStack>
                <Text color={OCEAN_TOKENS.muted} fontWeight="650" lineHeight="1.85" mb="4">
                  Nếu bài viết này chạm đúng điều bạn đang cần, hãy chọn một vùng học nhỏ bên dưới. Poo đã chuẩn bị sẵn các phao học miễn phí để bạn thử ngay hôm nay.
                </Text>
  ...
  return (
    <Box textAlign="center" bg="linear-gradient(135deg, rgba(31,111,214,0.95), rgba(91,188,235,0.9))" color="white" borderRadius="3xl" p={{ base: '5', md: '7' }} boxShadow="0 24px 70px rgba(31,111,214,0.22)">
      ...
      <Text maxW="720px" mx="auto" lineHeight="1.8" fontWeight="700" opacity="0.94" mb="5">
        Chỉ cần mở một bài, nói thử một câu hoặc ôn lại vài từ. Poo không cần bạn hoàn hảo; Poo chỉ cần bạn bơi thêm một nhịp nhỏ hôm nay.
      </Text>
      <HStack justify="center" gap="3" wrap="wrap">
        <Button as={RouterLink} to="/learning-path" borderRadius="full" bg="white" color={OCEAN_TOKENS.deepBlue} size="lg" _hover={{ bg: '#E8F4FF' }}>
          Vào lộ trình học
        </Button>
        <Button as={RouterLink} to="/shadowing" borderRadius="full" bg="rgba(255,255,255,0.16)" color="white" size="lg" border="1px solid rgba(255,255,255,0.62)" _hover={{ bg: 'rgba(255,255,255,0.24)' }}>
          Thử shadowing
        </Button>
      </HStack>
    </Box>
    ...
    <Box bg="linear-gradient(180deg, rgba(232,244,255,0.74), rgba(248,252,255,0.95))" minH="calc(100vh - 68px)" px={{ base: '3', md: '5' }} py={{ base: '4', md: '7' }}>
      <Container maxW="760px" px="0">
        <Box textAlign="center" bg="rgba(255,255,255,0.88)" border="1px solid" borderColor="rgba(186,230,253,0.86)" borderRadius="3xl" p={{ base: '6', md: '9' }} boxShadow="0 24px 70px rgba(31,111,214,0.12)">
          ...
          <Text as="h1" fontSize={{ base: '2xl', md: '4xl' }} fontWeight="950" color={OCEAN_TOKENS.text} mb="3">
            Bài viết này bơi lạc rồi
          </Text>
          <Text color={OCEAN_TOKENS.muted} fontWeight="700" lineHeight="1.8" mb="5">
            Poo chưa tìm thấy bài viết bạn muốn đọc. Mình quay lại blog để chọn một chiếc phao khác nhé.
          </Text>
          <Button as={RouterLink} to="/blog" borderRadius="full" bg={OCEAN_TOKENS.deepBlue} color="white" _hover={{ bg: OCEAN_TOKENS.oceanBlue }}>
            Về blog PooEnglish
          </Button>
        </Box>
      </Container>
