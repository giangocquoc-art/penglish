...
import { getUnifiedLessonContentDepth, type UnifiedLessonDepthLayerKey, type UnifiedSkillDepthKey } from '../lib/p-english/unifiedLessonEngine';
...
import { DynamicGuidedLessonFlow, getPracticeModeIcon, getPracticeModeLabel } from '../components/p-english/DynamicGuidedLessonFlow';
...
const LESSON_DEPTH_SKILL_LABELS: Record<UnifiedSkillDepthKey, string> = {
  vocabulary: 'Từ',
  grammar: 'Rule',
  reading: 'Đọc',
  listening: 'Nghe',
...
const LESSON_DEPTH_LAYER_TONES: Record<UnifiedLessonDepthLayerKey, { bg: string; color: string }> = {
  objective: { bg: '#EFF6FF', color: '#1D4ED8' },
  chunk: { bg: '#ECFDF5', color: '#047857' },
  pattern: { bg: '#F5F3FF', color: '#6D28D9' },
  context: { bg: '#FFF7ED', color: '#C2410C' },
  audio: { bg: '#E0F2FE', color: '#0369A1' },
  output: { bg: '#FEF3C7', color: '#B45309' },
  assessment: { bg: '#FEE2E2', color: '#B91C1C' },
  review: { bg: '#F0FDF4', color: '#15803D' },
  mistake: { bg: '#FFF1F2', color: '#BE123C' },
  multimodal: { bg: '#F0FDFA', color: '#0F766E' },
};
...
export function LessonPage() {
  ...
  const lessonContentDepth = useMemo(() => (lesson ? getUnifiedLessonContentDepth(lesson.id) : null), [lesson]);
  ...
  useGSAP(
    () => {
      ...
      if (reducedMotion) {
        safeGsapSet([hero, ...Array.from(metrics), coach, finalChallenge, ...Array.from(sparks)].filter(Boolean), { autoAlpha: 1, x: 0, y: 0, scale: 1, rotate: 0 });
        return;
      }
  ...
  return (
    <OceanPageShell data-testid="lesson-mobile-root" ref={pageRef} variant="dashboard" ambientWhalePreset="lesson" overlayStrength="medium" glassIntensity="clear" minH="calc(100vh - 72px)" px={{ base: '3', md: '6' }} pt={{ base: '2', md: '0' }} pb={{ base: 'calc(var(--penglish-mobile-safe-bottom) + 72px)', lg: '12' }} overflowX="hidden" sx={{ scrollPaddingBottom: 'calc(var(--penglish-mobile-safe-bottom) + 96px)' }}>
      <Box maxW="1180px" mx="auto" minW="0" pb={{ base: '112px', md: '0' }}>
        ...
        <Box className="lesson-hero-polish penglish-glass-card" bg={{ base: 'rgba(255,255,255,0.86)', md: 'rgba(255,255,255,0.78)' }} border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '3', md: '8' }} mb={{ base: '3', md: '5' }} overflow="hidden" position="relative" boxShadow="0 14px 34px rgba(37, 99, 235, 0.07)" backdropFilter="blur(14px) saturate(1.1)" willChange="transform, opacity">
          ...
          <VStack align="start" gap={{ base: '2', md: '5' }} position="relative">
            ...
            {learningLocked ? (
              ...
              <Button as={Link} to={firstPracticeUrl} display={{ base: 'none', md: 'inline-flex' }} bg={COLORS.primary} color="white" borderRadius="full" size="lg" leftIcon={<Icon as={getPracticeModeIcon(firstPracticeMode)} />} _hover={{ bg: '#1D4ED8' }}>
                Luyện {getPracticeModeLabel(firstPracticeMode)}
              </Button>
            ) : (
        ...
        {lessonContentDepth ? (
          <Box data-testid="lesson-content-depth-panel" className="penglish-glass-card" bg="rgba(239,246,255,0.76)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '3', md: '5' }} mb="5" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)" display={{ base: 'none', md: 'block' }}>
            <Flex justify="space-between" align={{ base: 'start', lg: 'center' }} direction={{ base: 'column', lg: 'row' }} gap="4" mb="4">
              <Box minW="0">
                <HStack gap="2" wrap="wrap" mb="1">
                  <Tag borderRadius="full" bg="white" color={lessonContentDepth.depthScore >= 68 ? '#047857' : '#B45309'} fontWeight="950"><TagLabel>{lessonContentDepth.depthScore}/100</TagLabel></Tag>
                  <Tag borderRadius="full" bg="#DBEAFE" color={COLORS.primary} fontWeight="950"><TagLabel>{lessonContentDepth.depthLabelVi}</TagLabel></Tag>
                </HStack>
                <Text fontWeight="950" color={COLORS.text} fontSize="xl">Bản đồ data của bài học</Text>
                <Text mt="1" color={COLORS.muted} fontWeight="750" lineHeight="1.55">{lessonContentDepth.recommendedStudyPathVi}</Text>
              </Box>
              <Button as={Link} to={firstPracticeUrl} borderRadius="full" bg={COLORS.primary} color="white" _hover={{ bg: '#1D4ED8' }} flexShrink={0}>
                Luyện theo data gợi ý
              </Button>
            </Flex>
            <SimpleGrid columns={{ base: 2, lg: 4 }} gap="3" mb="4">
              <Metric value={lessonContentDepth.vocabularyItems} label="vocab/flashcard" />
              <Metric value={lessonContentDepth.grammarItems} label="rule/pattern" />
              <Metric value={lessonContentDepth.listeningItems} label="nghe/hội thoại" />
              <Metric value={lessonContentDepth.writingItems} label="viết/gõ câu" />
            </SimpleGrid>
            <HStack gap="1.5" wrap="wrap">
              {lessonContentDepth.availableSkills.map((skill) => (
                <Tag key={skill} size="sm" borderRadius="full" bg="#ECFDF5" color="#047857" fontWeight="850">Có {LESSON_DEPTH_SKILL_LABELS[skill]}</Tag>
              ))}
              {lessonContentDepth.missingSkills.map((skill) => (
                <Tag key={skill} size="sm" borderRadius="full" bg="#FFF7ED" color="#C2410C" fontWeight="850">Thiếu {LESSON_DEPTH_SKILL_LABELS[skill]}</Tag>
              ))}
            </HStack>
            <Text mt="3" color={COLORS.primary} fontSize="sm" fontWeight="900">Đề xuất data: {lessonContentDepth.recommendedDataActionVi}</Text>
          </Box>
        ) : null}

        {lessonContentDepth ? (
          <Box data-testid="lesson-10-depth-layers-panel" className="penglish-glass-card" bg="rgba(255,255,255,0.78)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '3', md: '5' }} mb="5" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)" display={{ base: 'none', md: 'block' }}>
            <Flex justify="space-between" align={{ base: 'start', lg: 'center' }} direction={{ base: 'column', lg: 'row' }} gap="4" mb="4">
              <Box minW="0">
                <HStack gap="2" wrap="wrap" mb="1">
                  <Tag borderRadius="full" bg="#ECFDF5" color="#047857" fontWeight="950"><TagLabel>{lessonContentDepth.strongLayerCount}/10 lớp mạnh</TagLabel></Tag>
                  {lessonContentDepth.weakestLayer ? <Tag borderRadius="full" bg="#FFF7ED" color="#C2410C" fontWeight="950"><TagLabel>Yếu nhất: {lessonContentDepth.weakestLayer.labelVi}</TagLabel></Tag> : null}
                  {lessonContentDepth.strongestLayer ? <Tag borderRadius="full" bg="#DBEAFE" color={COLORS.primary} fontWeight="950"><TagLabel>Mạnh nhất: {lessonContentDepth.strongestLayer.labelVi}</TagLabel></Tag> : null}
                </HStack>
                <Text fontWeight="950" color={COLORS.text} fontSize="xl">10 lớp chiều sâu data bài học</Text>
                <Text mt="1" color={COLORS.muted} fontWeight="750" lineHeight="1.55">{lessonContentDepth.nextLayerActionVi}</Text>
              </Box>
              <Button as={Link} to={firstPracticeUrl} borderRadius="full" bg={COLORS.green} color="white" _hover={{ bg: '#16A34A' }} flexShrink={0}>
                Luyện lớp data mạnh nhất
              </Button>
            </Flex>
            <SimpleGrid columns={{ base: 2, lg: 5 }} gap="3">
              {lessonContentDepth.depthLayers.map((layer) => {
                const tone = LESSON_DEPTH_LAYER_TONES[layer.key];
                return (
                  <Box key={layer.key} border="1px solid" borderColor={layer.isStrong ? '#BBF7D0' : '#FED7AA'} borderRadius="2xl" bg={tone.bg} p="3" minW="0">
                    <HStack justify="space-between" align="start" gap="2">
                      <Box minW="0">
                        <Text color={tone.color} fontWeight="950" noOfLines={1}>{layer.labelVi}</Text>
                        <Text mt="1" color={COLORS.text} fontSize="2xl" fontWeight="950">{layer.score}</Text>
                      </Box>
                      <Tag size="sm" borderRadius="full" bg="white" color={tone.color} fontWeight="900"><TagLabel>{layer.itemCount}</TagLabel></Tag>
                    </HStack>
                    <Text mt="2" color={COLORS.muted} fontSize="xs" fontWeight="800" noOfLines={2}>{layer.evidenceVi}</Text>
                    {!layer.isStrong ? <Text mt="2" color="#C2410C" fontSize="xs" fontWeight="850" noOfLines={2}>{layer.actionVi}</Text> : null}
                  </Box>
                );
              })}
            </SimpleGrid>
