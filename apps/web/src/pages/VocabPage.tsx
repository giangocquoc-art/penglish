...
import { AlertCircle, BookOpen, CheckCircle, ChevronDown, Clock3, Heart, Layers, MessageCircle, Mic2, RotateCcw, Search, Star, Target, Volume2 } from 'lucide-react';
...
const DESKTOP_WORD_BATCH_SIZE = 12;
...
const WordActions = memo(function WordActions({ item, onStatusChange, onClearStatus, onSpeakWord }: { item: VocabularyReviewItem } & WordActionHandlers) {
  ...
  return (
    <HStack data-testid={`vocab-status-chips-${item.wordId}`} wrap="wrap" gap="2">
      <IconButton data-testid={`vocab-speak-${item.wordId}`} aria-label={`Nghe ${item.term}`} icon={<Icon as={Volume2} />} size="sm" minW={{ base: '44px', md: '32px' }} minH={{ base: '44px', md: '32px' }} borderRadius="full" variant="outline" onClick={() => onSpeakWord(item.term)} />
      <Button data-testid={`vocab-review-link-${item.wordId}`} as={Link} to={reviewUrl} size="sm" minH={{ base: '44px', md: '32px' }} borderRadius="full" leftIcon={<Icon as={BookOpen} />} variant="outline">
        Ôn lại
      </Button>
      <Button data-testid={`vocab-mark-known-${item.wordId}`} size="sm" minH={{ base: '44px', md: '32px' }} borderRadius="full" leftIcon={<Icon as={CheckCircle} />} bg="#DCFCE7" color="#166534" onClick={() => onStatusChange(item.wordId, 'known')}>
        Đã nhớ
      </Button>
      <Button data-testid={`vocab-mark-review-${item.wordId}`} size="sm" minH={{ base: '44px', md: '32px' }} borderRadius="full" leftIcon={<Icon as={RotateCcw} />} bg="#FFF7ED" color="#9A3412" onClick={() => onStatusChange(item.wordId, 'review')}>
        Cần ôn
      </Button>
      <Button data-testid={`vocab-mark-difficult-${item.wordId}`} size="sm" minH={{ base: '44px', md: '32px' }} borderRadius="full" bg="#FEE2E2" color="#991B1B" onClick={() => onStatusChange(item.wordId, 'difficult')}>
        Hay sai
      </Button>
      <Button data-testid={`vocab-clear-status-${item.wordId}`} size="sm" minH={{ base: '44px', md: '32px' }} borderRadius="full" variant="ghost" color={COLORS.muted} onClick={() => onClearStatus(item.wordId)}>
        Xóa khỏi sổ
      </Button>
    </HStack>
  );
  ...
  return (
    <Box data-testid="vocab-mobile-card" className="penglish-glass-card vocab-word-card" bg="rgba(255,255,255,0.90)" backdropFilter={{ base: 'none', md: 'blur(6px) saturate(1.02)' }} border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '3.5', md: '5' }} boxShadow={{ base: '0 4px 14px rgba(31, 111, 214, 0.05)', md: '0 10px 24px rgba(31, 111, 214, 0.06)' }} minW="0" overflow="hidden" style={{ contentVisibility: 'auto', containIntrinsicSize: '420px' }}>
      ...
      <Box as="details" mt="4" bg="#F0F9FF" border="1px solid" borderColor="#BAE6FD" borderRadius="2xl" p="4" data-testid={`vocab-learning-hint-${item.wordId}`}>
        <Flex as="summary" cursor="pointer" minH="44px" align="center" fontSize="xs" fontWeight="700" color={COLORS.primary} textTransform="uppercase" letterSpacing="0.08em">Mẹo học nhóm {item.learningGroup.cefrLevel}</Flex>
...
export function VocabPage() {
  ...
  const [desktopVisibleCount, setDesktopVisibleCount] = useState(DESKTOP_WORD_BATCH_SIZE);
  const [notebookOpen, setNotebookOpen] = useState(() => typeof window === 'undefined' || window.matchMedia('(min-width: 768px)').matches);
  ...
  useEffect(() => {
    ...
    setDesktopVisibleCount(DESKTOP_WORD_BATCH_SIZE);
  }, [deferredQuery, lessonFilter, statusFilter]);

  useEffect(() => {
    const desktopQuery = window.matchMedia('(min-width: 768px)');
    const openNotebookOnDesktop = (event: MediaQueryListEvent) => {
      if (event.matches) setNotebookOpen(true);
    };
    desktopQuery.addEventListener('change', openNotebookOnDesktop);
    return () => desktopQuery.removeEventListener('change', openNotebookOnDesktop);
  }, []);
  ...
  const visibleDesktopWords = useMemo(() => filtered.slice(0, desktopVisibleCount), [desktopVisibleCount, filtered]);
  ...
  if (vocabularyState.error) {
    return (
      <OceanPageShell data-testid="vocab-mobile-root" variant="vocab" overlayStrength="strong" minH="calc(100vh - 72px)" px={{ base: '3', md: '6' }} py={{ base: '2', md: '5' }} pb={{ base: 'var(--penglish-mobile-safe-bottom)', lg: '12' }} overflowX="hidden">
        <Box maxW="760px" mx="auto" className="penglish-glass-card" bg="rgba(255,255,255,0.88)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)" overflow="hidden">
          <PooSystemState
            variant="error"
            headingAs="h1"
            title="Poo chưa mở được sổ từ vựng"
            description="Dữ liệu từ vựng chưa tải về được. Bạn tải lại để tiếp tục đúng danh sách đang học nhé."
            actionLabel="Tải lại sổ từ"
            onAction={refresh}
          />
        </Box>
      </OceanPageShell>
      ...
      <Box maxW="1480px" mx="auto" minW="0">
        <Box className="penglish-glass-card vocab-review-first-card" bg="rgba(255,255,255,0.82)" backdropFilter={{ base: 'none', md: 'blur(8px) saturate(1.04)' }} border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '4', md: '6' }} mb={{ base: '3', md: '4' }} boxShadow="0 10px 24px rgba(37, 99, 235, 0.06)" position="relative" overflow="hidden">
          <Flex justify="space-between" align={{ base: 'stretch', md: 'center' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '3', md: '5' }}>
            <Box minW="0">
              <Text as="h1" fontSize={{ base: '2xl', md: '4xl' }} lineHeight="1.08" fontWeight="850" color={COLORS.text}>Hôm nay ôn 10 từ nhé?</Text>
        ...
        <Box as="details" open={notebookOpen} onToggle={(event) => setNotebookOpen(event.currentTarget.open)} className="vocab-notebook-details" data-testid="vocab-notebook-details">
          <Flex as="summary" cursor="pointer" color={COLORS.primary} mb="3" listStyleType="none" minH="52px" px="4" py="2.5" borderRadius="2xl" bg="rgba(255,255,255,0.68)" border="1px solid" borderColor="#BAE6FD" align="center" justify="space-between" gap="3" _hover={{ bg: 'rgba(255,255,255,0.86)' }} _focusVisible={{ outline: '3px solid', outlineColor: COLORS.primary, outlineOffset: '2px' }}>
            <Box minW="0">
              <Text fontWeight="900">{notebookOpen ? 'Thu gọn sổ từ vựng' : 'Mở sổ từ vựng'}</Text>
              <Text mt="0.5" color={COLORS.muted} fontSize="xs" fontWeight="700">Tìm, lọc, nghe phát âm và đánh dấu những từ bạn cần ôn.</Text>
            </Box>
            <Icon as={ChevronDown} flexShrink={0} transform={notebookOpen ? 'rotate(180deg)' : 'none'} transition="transform .18s ease" />
          </Flex>
        ...
        {items.length === 0 ? (
          <Box className="penglish-glass-card" bg="rgba(255,255,255,0.78)" backdropFilter="blur(14px) saturate(1.1)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)">
            <PooSystemState variant="empty" title="Sổ học đang chờ từ đầu tiên" description="Học một bài ngắn để Poo giữ lại từ và câu cần ôn cho bạn nhé." actionLabel="Học bài A1 đầu tiên" actionTo="/learning-path/lesson/unit-1-greetings/unit-1-greetings-vocabulary-0" />
          </Box>
        ) : filtered.length === 0 ? (
          ...
          <>
            ...
            {desktopHiddenCount > 0 ? (
              <Flex display={{ base: 'none', lg: 'flex' }} mb="3" bg="#EFF6FF" border="1px solid" borderColor="#BAE6FD" borderRadius="2xl" p="3" align="center" justify="space-between" gap="3">
                <Text color={COLORS.muted} fontWeight="700" minW="0">
                ...
                <Button flexShrink={0} borderRadius="full" colorScheme="blue" onClick={() => setDesktopVisibleCount((count) => count + DESKTOP_WORD_BATCH_SIZE)}>
                  Xem thêm {Math.min(DESKTOP_WORD_BATCH_SIZE, desktopHiddenCount)} từ
                </Button>
              </Flex>
            ) : null}
