...
const routeChecks = [
  { slug: 'home', path: '/', expected: ['PooEnglish'] },
  { slug: 'learning-path', path: '/learning-path', expected: ['Lộ trình học'] },
  {
    slug: 'unit-1',
    ...
    expected: [
      ...
      { label: 'guided lesson step', any: ['Bước 1: Nghe', 'Nghe câu này'] },
    ],
  },
  {
    slug: 'new-lesson',
    ...
    expected: [
      { label: 'A1 classmate lesson title', any: ['Gặp bạn mới trong lớp', 'Meeting a new classmate'] },
      { label: 'guided listening step', any: ['Bước 1: Nghe', 'Hi, I am new here.', 'Is this seat free?'] },
    ],
  },
  { slug: 'english-speed', path: '/english-speed', expected: ['Đọc nhanh cùng Poo', 'Nói rõ và đều'] },
  { slug: 'shadowing', path: '/shadowing', expected: ['Shadowing', 'Poo'] },
  { slug: 'words', path: '/words', expected: ['Hôm nay ôn', 'Mở sổ tay'] },
];
...
async function main() {
  ...
  const explanationVisible = /Đúng rồi|Đáp án đúng|Vì sao đúng|Đọc transcript/i.test(checkedText);
  ...
  for (let guard = 0; guard < 7; guard += 1) {
    ...
    if (/Tổng kết (?:Listening|luyện nghe)|Hoàn thành luyện nghe/i.test(text)) break;
    await page.keyboard.press('1');
    await page.keyboard.press('Enter');
  ...
  const summaryVisible = /Tổng kết (?:Listening|luyện nghe)|Hoàn thành luyện nghe/i.test(summaryText);
  ...
  if (failedRouteTextChecks > 0 || criticalMessages.length > 0 || failedRequests.length > 0 || !explanationVisible || !enterAdvanced || !summaryVisible || !detailedAttempts || !dailyRewardSaved) {
    process.exitCode = 1;
  }
}
