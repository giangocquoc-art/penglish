...
import {
  User, Image as ImageIcon, Star, Package, Upload, Eye, Plus, Check, Lock, Waves, Coins, Gift,
  ShieldCheck, Palette, Volume2,
} from 'lucide-react';
...
type ShopItem = {
  id: string;
  name: string;
  type: 'avatar' | 'background' | 'streak_freeze' | 'theme' | 'sound' | 'special' | string;
...
function imgFor(item: ShopItem): string | null {
  ...
  const ext = Number.isFinite(numericId) ? ASSET_EXT[numericId] : undefined;
  if (ext) return `/assets/shop/${numericId}.${ext}`;
  return item.imageUrl ?? null;
}

function iconFor(item: ShopItem) {
  if (item.visualKey === 'streak-shield' || item.type === 'streak_freeze') return ShieldCheck;
  if (item.visualKey === 'theme-blue' || item.type === 'theme') return Palette;
  if (item.visualKey === 'typing-sfx' || item.type === 'sound') return Volume2;
  return Gift;
}
      ...
      <Flex align="center" justify="space-between" gap="4" flexWrap="wrap">
        <HStack gap="4">
          <Flex
            ...
            boxShadow="md"
          >
            <Icon as={Coins} boxSize="8" color="#B45309" aria-hidden="true" />
          </Flex>
        ...
        >
          <Icon as={Plus} mr="2" /> Nạp thêm · Sắp có
        </Button>
      </Flex>
    </Box>
  );
...
function ItemTile({
  ...
  item: ShopItem; owned: boolean; index: number;
}) {
    ...
    >
      ...
      >
        {src ? (
          ...
          <Icon
            as={ItemIcon}
            boxSize="12"
            color={item.type === 'streak_freeze' ? '#0891B2' : item.type === 'theme' ? '#1F6FD6' : '#7C3AED'}
            aria-hidden="true"
          />
        )}
      </Flex>
      <VStack align="stretch" gap="2" p="3">
        ...
        <HStack justify="space-between">
          ...
          >
            <Icon as={Coins} boxSize="4" color="#B45309" aria-hidden="true" />
...
function UploadTile() {
  return (
    ...
    >
      ...
      <Text fontSize="sm" fontWeight="700" textAlign="center">Tải ảnh của bạn</Text>
      <Text fontSize="xs" color="#52667A" textAlign="center">Sắp có, Poo đang chuẩn bị nhé</Text>
    </Flex>
  );
}
...
function ShopLoadErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <PooSystemState
      variant="error"
      title="Poo chưa mở được cửa hàng"
      description="Kết nối cửa hàng đang chậm. Bạn tải lại để Poo kiểm tra vật phẩm và bọt biển nhé."
      actionLabel="Tải lại cửa hàng"
      onAction={onRetry}
    />
  );
}
...
export function ShopPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [itemsResult, coinsResult, inventoryResult] = await Promise.allSettled([
      get<any>('/shop/items'),
      get<any>('/shop/coins'),
      get<any>('/shop/inventory'),
    ]);

    if (itemsResult.status === 'fulfilled') {
      const r = itemsResult.value;
    ...
    if (inventoryResult.status === 'fulfilled') {
      const r = inventoryResult.value;
      const list = Array.isArray(r) ? r : (r?.data ?? r?.body ?? []);
      const ids = list
        .map((x: any) => (typeof x === 'string' || typeof x === 'number' ? String(x) : x?.itemId ?? x?.id))
        .filter((v: any) => v != null)
        .map(String);
      setInventory(ids);
    } else {
    ...
    // A down API must not look like a stocked-but-empty shop.
    setLoadFailed(itemsResult.status === 'rejected');
    setLoading(false);
  }, []);
  ...
  return (
    <Box px="6" pb="10" maxW="1400px" mx="auto">
      <Flex justify="space-between" align={{ base: 'start', sm: 'center' }} direction={{ base: 'column', sm: 'row' }} mb="5" flexWrap="wrap" gap="3">
        <Text as="h1" fontSize="2xl" fontWeight="800" color="gray.800">Cửa hàng</Text>
        <Badge display="inline-flex" alignItems="center" gap="2" border="1px solid #8FD8F7" color="#1F6FD6" bg="white" borderRadius="full" fontWeight="700" px="3" py="2" textTransform="none">
          <Icon as={Eye} /> Toàn cảnh sắp mở
        </Badge>
      </Flex>
      ...
      {loadFailed ? (
        <ShopLoadErrorState onRetry={() => void load()} />
      ) : loading ? (
        <PooSystemState variant="loading" title="Poo đang mở cửa hàng..." description="Poo đang kiểm tra vật phẩm và số bọt biển của bạn." />
      ) : filtered.length === 0 ? (
