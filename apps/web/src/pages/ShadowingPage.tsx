...
import { requestShadowingFeedback, type ShadowingApiFailure, type ShadowingApiResult, type ShadowingApiStatus, type ShadowingApiSuccess } from '../lib/p-english/shadowingApiClient';
...
const MIC_UNSUPPORTED_MESSAGE = 'Trình duyệt hoặc kết nối này chưa hỗ trợ micro (cần HTTPS và trình duyệt mới). Bạn thử Chrome và dùng mạng an toàn nha.';
const SPEECH_UNCLEAR_MESSAGE = 'Poo chưa nghe rõ câu này, thử nói chậm hơn một chút nha.';
const SPEECH_RECOGNITION_FALLBACK_MESSAGE = 'Trình duyệt này chưa hỗ trợ nhận diện giọng nói. Nên thử Chrome để Poo chấm phát âm nha.';
...
function getShadowingStatus(score: number, normalizedTarget: string, normalizedTranscript: string): ShadowingApiStatus {
  if (!normalizedTranscript) return 'retry';
  if (/\bpoo\b/.test(normalizedTarget) && /\bboo\b/.test(normalizedTranscript)) return 'nearCorrect';
  if (score >= 88) return 'correct';
...
function compareShadowingTranscript(targetText: string, attemptText: string): ShadowingApiSuccess {
  ...
  const status = getShadowingStatus(score, normalizedTarget, normalizedTranscript);
...
export function ShadowingPracticePage() {
  ...
  const returnToToday = searchParams.get('returnTo') === '/today';
  const sessionQuery = returnToToday ? '?returnTo=%2Ftoday' : '';
  ...
  const sendRecordingForFeedback = async (blob: Blob) => {
    ...
    if (result.ok) {
      ...
      setRecordMessage(result.allowContinue ? result.coachMessage : `${result.coachMessage} Poo đã giữ câu này ở phần ôn cùng Poo để bạn quay lại nhẹ nhàng.`);
      if (result.allowContinue) markSentenceCompleted(selectedSentence?.id);
    } else {
      ...
      if (fallback) {
        ...
        setApiError(null);
        setRecordingStatus('result');
        setRecordMessage(`${fallback.coachMessage} Poo góp ý từ câu bạn nhập.${fallback.allowContinue ? '' : ' Poo đã lưu câu này vào phần Ôn tập để bạn luyện lại.'}`);
  ...
  const startRecording = async () => {
    if (typeof window === 'undefined' || !navigator.mediaDevices?.getUserMedia) {
      const failure: ShadowingApiFailure = { ok: false, error: 'NO_AUDIO', message: MIC_UNSUPPORTED_MESSAGE };
      setApiFeedback(null);
      setApiError(failure);
      setRecordingStatus('error');
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        if (blob.size > 0) {
          // No live SpeechRecognition on this browser — hand the recording to the
          // server-side shadowing-feedback endpoint so learners still get real feedback.
          void sendRecordingForFeedback(blob);
        } else {
          const failure: ShadowingApiFailure = { ok: false, error: 'EMPTY_AUDIO', message: SPEECH_RECOGNITION_FALLBACK_MESSAGE };
          setApiFeedback(null);
          setApiError(failure);
          setRecordingStatus('error');
          setRecordMessage(failure.message);
  ...
  const createCustomPractice = () => {
    ...
    navigate(`/shadowing/practice/${next.id}${sessionQuery}`);
  };
    ...
    <Box data-testid="shadowing-feedback-panel" className="shadowing-feedback-panel penglish-glass-card" p={{ base: '4', md: '5' }} borderRadius="3xl" bg="rgba(255,255,255,0.78)" border="1px solid" borderColor="#BAE6FD" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)" backdropFilter="blur(14px) saturate(1.1)" position="relative" overflow="hidden">
      ...
      {apiFeedback ? (
        ...
        <Box p="4" borderRadius="2xl" bg="rgba(232,244,255,0.7)" border="1px solid" borderColor="#BAE6FD">
          ...
          <Text mt="1" fontSize="sm" color={COLORS.muted} lineHeight="1.7">Nghe trước một lần, nói lại theo nhịp, rồi xem góp ý nhẹ. Chỉ cần nói câu ngắn, đúng ý, rõ nhịp. Không cần nói hoàn hảo.</Text>
        </Box>
      )}
    </Box>
  );
  ...
  return (
    <OceanPageShell variant="shadowing" overlayStrength="medium" data-testid="shadowing-mobile-root" ref={rootRef} minH="calc(100vh - 68px)" px={{ base: '3', md: '5' }} pb={{ base: 'calc(var(--penglish-mobile-safe-bottom) + 80px)', lg: '10' }} pt={{ base: '3', md: 'calc(68px + 1.25rem)', lg: 'calc(72px + 1.5rem)' }} overflowX="hidden" scrollMarginTop="96px" sx={{ scrollPaddingTop: '96px', scrollPaddingBottom: 'calc(var(--penglish-mobile-safe-bottom) + 96px)' }}>
      <Box maxW="1180px" mx="auto" minW="0" pb={{ base: '128px', lg: '0' }}>
        ...
        <Flex direction={{ base: 'column', lg: 'row' }} gap={{ base: '3', md: '5' }} minW="0" align="start">
          <VStack align="stretch" gap={{ base: '3', md: '4' }} minW="0" flex="1">
            <Box data-testid="shadowing-practice-card" className="shadowing-practice-card penglish-glass-card" p={{ base: '3', md: '4' }} borderRadius="3xl" bg="rgba(255,255,255,0.78)" border="1px solid" borderColor="#BAE6FD" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)" minW="0" willChange="transform, opacity" backdropFilter="blur(14px) saturate(1.1)" position="relative" overflow="hidden">
              ...
              <Flex justify="space-between" align={{ base: 'stretch', sm: 'start' }} gap="3" direction={{ base: 'column', sm: 'row' }} mb="3">
                <Box minW="0" flex="1">
                ...
                <HStack gap="2" wrap="wrap" justify={{ base: 'start', sm: 'end' }}>
            ...
            {returnToToday && completedCount >= 2 ? (
              <Box data-testid="shadowing-today-session-ready" p={{ base: '3.5', md: '4' }} borderRadius="2xl" bg="linear-gradient(135deg, #F0FDF4, #E8F4FF)" border="1px solid" borderColor="#86EFAC" role="status" aria-live="polite">
                <Flex gap="3" align={{ base: 'stretch', sm: 'center' }} justify="space-between" direction={{ base: 'column', sm: 'row' }}>
                  <HStack gap="3" align="center">
                    <Flex w="44px" h="44px" borderRadius="2xl" bg="#DCFCE7" color="#15803D" align="center" justify="center" flexShrink={0}><Icon as={CheckCircle2} boxSize="6" /></Flex>
                    <Box minW="0"><Text color={COLORS.text} fontWeight="950">Đã đủ 2 câu cho phiên hôm nay</Text><Text mt="0.5" color={COLORS.muted} fontSize="sm" fontWeight="750">Poo đã ghi lại lượt nói. Bạn quay về để xem tổng kết nhé.</Text></Box>
                  </HStack>
                  <Button as={Link} to="/today" data-testid="shadowing-return-to-today" borderRadius="full" bg={COLORS.deepBlue} color="white" _hover={{ bg: COLORS.oceanBlue }} flexShrink={0}>Xem tổng kết phiên</Button>
                </Flex>
              </Box>
            ) : null}
          ...
          <VStack align="stretch" gap={{ base: '3', md: '4' }} minW="0" w={{ base: '100%', lg: '340px' }} flexShrink={0} position={{ lg: 'sticky' }} top={{ lg: '96px' }} maxH={{ lg: 'calc(100vh - 120px)' }} overflowY={{ lg: 'auto' }} pb={{ base: '96px', lg: '2' }} pr={{ lg: '1' }}>
            <Box data-testid="shadowing-lesson-picker" as="details" open={typeof window !== 'undefined' ? window.innerWidth >= 1024 : false} className="shadowing-custom-card penglish-glass-card" p="4" borderRadius="3xl" bg="rgba(255,255,255,0.78)" border="1px solid" borderColor="#BAE6FD" boxShadow="0 12px 30px rgba(16, 42, 67, 0.05)" backdropFilter={{ base: 'none', md: 'blur(10px) saturate(1.04)' }}>
              <Flex as="summary" cursor="pointer" minH="44px" align="center" fontWeight="700" color={COLORS.text}>Tùy chọn</Flex>
            ...
            <Box data-testid="shadowing-transcript-panel" as="details" open={typeof window !== 'undefined' ? window.innerWidth >= 1024 : false} className="shadowing-transcript-panel penglish-glass-card" p="4" borderRadius="3xl" bg="rgba(255,255,255,0.78)" border="1px solid" borderColor="#BAE6FD" minW="0" willChange="transform, opacity" boxShadow="0 10px 24px rgba(31, 111, 214, 0.05)" backdropFilter={{ base: 'none', md: 'blur(10px) saturate(1.04)' }}>
              <Flex as="summary" cursor="pointer" minH="44px" align="center" fontWeight="700" color={COLORS.text}>Xem lời thoại</Flex>
            ...
            <Box data-testid="shadowing-custom-transcript" as="details" className="shadowing-custom-card penglish-glass-card" p="4" borderRadius="3xl" bg="rgba(248,252,255,0.78)" border="1px solid" borderColor={COLORS.border} boxShadow="0 10px 24px rgba(16, 42, 67, 0.04)" backdropFilter="blur(14px) saturate(1.1)">
              <Flex as="summary" cursor="pointer" minH="44px" align="center" fontWeight="700" color={COLORS.text}>Tự tạo lời thoại với Poo</Flex>
              ...
              <VStack align="stretch" gap="3">
                <FormControl>
                  ...
                  <Input data-testid="shadowing-custom-title-input" aria-label="Tiêu đề bài luyện" value={customTitle} onChange={(e) => setCustomTitle(e.target.value)} placeholder="Ví dụ: My daily English practice" bg="white" borderRadius="xl" />
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" fontWeight="700" color={COLORS.text}>Link video tham khảo (không bắt buộc)</FormLabel>
                  <Input data-testid="shadowing-custom-url-input" aria-label="Link video tham khảo" value={customUrl} onChange={(e) => setCustomUrl(e.target.value)} placeholder="Dán link video nếu bạn có" bg="white" borderRadius="xl" scrollMarginBottom="calc(var(--penglish-mobile-safe-bottom) + 96px)" />
                </FormControl>
                <FormControl>
                  <FormLabel fontSize="sm" fontWeight="700" color={COLORS.text}>Lời thoại - mỗi dòng là một câu</FormLabel>
                  <Textarea data-testid="shadowing-custom-transcript-textarea" aria-label="Lời thoại, mỗi dòng là một câu" value={customTranscript} onChange={(e) => setCustomTranscript(e.target.value)} placeholder={'Hello, my name is Anna.\nI am learning English every day.'} bg="white" borderRadius="xl" minH="130px" />
                </FormControl>
        ...
        <Box display={{ base: 'block', md: 'none' }} position="fixed" left="0" right="0" bottom="calc(var(--penglish-mobile-nav-height) + env(safe-area-inset-bottom))" zIndex="20" px="3" pt="3" pb="3" bg="linear-gradient(180deg, rgba(248,252,255,0), rgba(248,252,255,0.96) 34%, rgba(248,252,255,1))">
