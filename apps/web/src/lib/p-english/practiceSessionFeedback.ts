...
function getCoachLine(mode: LessonProgressMode, percentage: number) {
  if (percentage >= 80) {
    if (mode === 'flashcard') return 'Bạn nhớ mặt chữ khá tốt. Hãy chuyển sang quiz hoặc phản xạ để dùng từ trong câu.';
    ...
    return 'Bạn đã hoàn thành tốt lượt này. Giữ nhịp bằng một mode khác ngắn hơn.';
  }
  ...
  if (mode === 'match') return 'Các cặp còn nhầm nên được ôn bằng flashcard trước khi ghép lại.';
...
export function getPracticeSessionFeedback(input: PracticeSessionFeedbackInput): PracticeSessionFeedback {
  ...
  return {
    ...
    nextActionVi: weakCount > 0 ? 'Ưu tiên ôn mục yếu trước, sau đó mới chuyển mode tiếp theo.' : `Bước tiếp theo hợp lý: ${MODE_LABELS[nextMode]}.`,
    nextMode,
