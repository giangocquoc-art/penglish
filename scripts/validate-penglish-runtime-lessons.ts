...
function countGeneratedBridgeCoverage() {
  ...
  const generatedReadingLessons = allPEnglishLessons.filter((lesson) => lesson.id.startsWith('reading-'));
  ...
  return {
    ...
    generatedReadingLessonCount: generatedReadingLessons.length,
    readingLessonsWithThreeSentenceChunks: generatedReadingLessons.filter((lesson) => new Set(
      lesson.vocabulary
        .filter((item) => item.partOfSpeechOrType === 'sentence chunk')
        .map((item) => normalizeToken(item.term)),
    ).size >= 3).length,
    readingLessonsWithTwoListeningItems: generatedReadingLessons.filter((lesson) => new Set(lesson.listeningPractice.map((item) => normalizeToken(item.text))).size >= 2).length,
    readingLessonsWithThreeSpeakingPrompts: generatedReadingLessons.filter((lesson) => new Set(lesson.speakingReflexPrompts.map((item) => normalizeToken(item.expectedEnglish))).size >= 3).length,
  };
}
...
function toMarkdown(report: ReturnType<typeof createReport>) {
  ...
  return [
    ...
    `- Generated reading lessons with at least 3 sentence chunks: ${report.generatedGrammarBridgeCoverage.readingLessonsWithThreeSentenceChunks}/${report.generatedGrammarBridgeCoverage.generatedReadingLessonCount}`,
    `- Generated reading lessons with at least 2 distinct listening items: ${report.generatedGrammarBridgeCoverage.readingLessonsWithTwoListeningItems}/${report.generatedGrammarBridgeCoverage.generatedReadingLessonCount}`,
    `- Generated reading lessons with at least 3 distinct speaking prompts: ${report.generatedGrammarBridgeCoverage.readingLessonsWithThreeSpeakingPrompts}/${report.generatedGrammarBridgeCoverage.generatedReadingLessonCount}`,
    '',
