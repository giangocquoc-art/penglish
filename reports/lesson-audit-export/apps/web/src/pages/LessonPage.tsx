...
import { calculateLessonProgressSummary, getAvailableLessonProgressModes, getDueReviewItems, getWeakReviewItems, readLessonProgress, type LessonProgress } from '../lib/p-english/lesson-progress';
import { getPracticeRecommendation } from '../lib/p-english/practiceRecommendationEngine';
import { markLessonCompleted } from '../lib/p-english/local-progress';
...
function Section({
  ...
  return (
    <Box id={id} bg="rgba(255,255,255,0.9)" backdropFilter="blur(16px)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '5', md: '7' }} boxShadow="0 16px 40px rgba(31, 111, 214, 0.08)">
...
export function LessonPage() {
  ...
  if (!lesson) {
    return (
      <Box bg="linear-gradient(180deg, #DDF5FF 0%, #F8FCFF 48%, #FFFFFF 100%)" minH="calc(100vh - 72px)" px="6" py="8">
        <Box maxW="920px" mx="auto" bg="rgba(255,255,255,0.88)" backdropFilter="blur(16px)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p="8" boxShadow="0 18px 45px rgba(31, 111, 214, 0.10)">
  ...
  return (
    <Box ref={pageRef} bg="linear-gradient(180deg, #DDF5FF 0%, #F8FCFF 48%, #FFFFFF 100%)" minH="calc(100vh - 72px)" px={{ base: '4', md: '6' }} pb="12" overflowX="hidden">
      <Box maxW="1180px" mx="auto" minW="0">
        <HStack py="5" gap="2" color={COLORS.muted} fontSize="sm" wrap="wrap">
          <Link to="/home">Trang chủ</Link>
          <Text>/</Text>
          <Text>{lesson.unitTitle}</Text>
        </HStack>

        <Box className="lesson-hero-polish" bg="rgba(255,255,255,0.86)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '6', md: '8' }} mb="5" overflow="hidden" position="relative" boxShadow="0 18px 46px rgba(37, 99, 235, 0.09)" backdropFilter="blur(18px)" willChange="transform, opacity">
          ...
          <VStack align="start" gap="5" position="relative">
            <Button as={Link} to="/home" leftIcon={<Icon as={ArrowLeft} />} variant="ghost" color={COLORS.primary} px="0" _hover={{ bg: 'transparent' }}>
              Quay lại trang chủ
            </Button>
            <HStack wrap="wrap" gap="2">
              ...
              {lesson.skillTags.map((tag) => (
                <Tag key={tag} borderRadius="full" bg="white" color={COLORS.text} border="1px solid" borderColor={COLORS.border}><TagLabel>{tag}</TagLabel></Tag>
              ))}
            </HStack>
            <Box maxW="780px">
              <Text as="h1" fontSize={{ base: '3xl', md: '5xl' }} lineHeight="1.05" fontWeight="950" color={COLORS.text}>{lesson.titleVi}</Text>
              <Text mt="3" fontSize={{ base: 'lg', md: '2xl' }} fontWeight="700" color={COLORS.primary}>{lesson.titleEn}</Text>
              <Text mt="4" fontSize={{ base: 'md', md: 'lg' }} color={COLORS.muted}>{lesson.subtitle}</Text>
            </Box>
            {learningLocked ? (
              <LearningLockedLessonNotice state={heartsState} />
            ) : firstPracticeMode ? (
              <Button as={Link} to={firstPracticeUrl} bg={COLORS.primary} color="white" borderRadius="full" size="lg" leftIcon={<Icon as={PRACTICE_MODE_ICONS[firstPracticeMode]} />} _hover={{ bg: '#1D4ED8' }}>
                Luyện {PRACTICE_MODE_LABELS[firstPracticeMode]}
              </Button>
            ) : (
              <Button as={Link} to="/learning-path" bg={COLORS.primary} color="white" borderRadius="full" size="lg" _hover={{ bg: '#1D4ED8' }}>
                Xem lộ trình
              </Button>
            )}
          </VStack>
        ...
        <Box bg="rgba(255,255,255,0.88)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '5', md: '6' }} mb="5" boxShadow="0 14px 34px rgba(31, 111, 214, 0.07)">
        ...
        <Box className="lesson-whale-coach" bg="rgba(248,252,255,0.82)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '4', md: '5' }} mb="6" boxShadow="0 12px 30px rgba(31, 111, 214, 0.06)" overflow="hidden" position="relative" willChange="transform, opacity">
          <Box position="absolute" inset="0" bg="radial-gradient(circle at 92% 28%, rgba(91,188,235,0.10), transparent 24%)" />
          <Flex position="relative" align="center" justify="space-between" gap={{ base: '3', md: '5' }} direction={{ base: 'row', md: 'row' }}>
            <HStack align="center" gap={{ base: '3', md: '4' }} flex="1" minW="0">
              ...
              <Box minW="0">
                ...
                <Text mt="1" color={COLORS.muted} fontSize={{ base: 'sm', md: 'md' }} lineHeight="1.65">
        ...
        <VStack align="stretch" gap="5" display={{ base: 'none', lg: 'flex' }}>
          {progressSummary ? <LessonProgressDashboard summary={progressSummary} hasProgress={Boolean(progress)} /> : null}
          {progressSummary ? <SmartReviewCard lessonId={lesson.id} dueCount={dueReviewItems.length} weakCount={weakReviewItems.length} nextReviewAt={progressSummary.nextReviewAt} /> : null}

          <Section id="objectives" title="Mục tiêu sau bài học" icon={Target}>
            <SimpleGrid columns={{ base: 1, md: 2 }} gap="3">
              {lesson.learningObjectives.map((objective) => (
                <HStack key={objective} align="start" bg="#F8FAFC" border="1px solid" borderColor={COLORS.border} borderRadius="xl" p="4">
          ...
          <Section id="vocabulary" title="Vocabulary grid" subtitle="Các cụm nên học như chunk, không dịch từng chữ." icon={BookOpen}>
            <SimpleGrid columns={{ base: 1, md: 2, xl: 3 }} gap="4">
              {lesson.vocabulary.map((item) => (
                <Box key={item.id} border="1px solid" borderColor={COLORS.border} borderRadius="2xl" p="4" bg="#FFFFFF">
                  <Box mb="3">
                    <AnimatedWordScene
          ...
          <Section id="patterns" title="Sentence patterns" subtitle="Mẫu câu trọng tâm để thay tên/nơi chốn nhanh." icon={MessageCircle}>
          ...
          <Section id="dialogues" title="Mini dialogues" subtitle="Đọc theo vai A/B rồi shadowing từng câu." icon={Mic2}>
          ...
          <Section id="grammar" title="Grammar notes" subtitle="Mực Mơ giữ ngữ pháp ở mức cần dùng để phản xạ." icon={HelpCircle}>
          ...
          <Section id="pronunciation" title="Pronunciation" subtitle="Tập âm và nhịp đọc của các cụm thường dùng." icon={Volume2}>
          ...
          <Section id="completion" title="Completion criteria" subtitle="Tiêu chí hoàn thành được tính theo dữ liệu bài học và các mode đang có." icon={CheckCircle2}>
          ...
          <Box className="lesson-final-challenge" bg={COLORS.text} color="white" borderRadius="3xl" p={{ base: '6', md: '8' }} position="relative" overflow="hidden" willChange="transform, opacity">
    ...
    <Section id="lesson-progress" title="Tiến độ bài học" subtitle="Theo dõi các mode đang có dữ liệu cho bài học này trên thiết bị của bạn." icon={CheckCircle2}>
      ...
      <SimpleGrid columns={{ base: 1, sm: 2, lg: 4 }} gap="3">
        {summary.modeSummaries.map((item) => {
          const isDone = item.status === 'Đã hoàn thành';
          const isStarted = item.status === 'Đang học';
          const statusColor = isDone ? COLORS.green : isStarted ? COLORS.amber : COLORS.muted;
          const statusBg = isDone ? '#F0FDF4' : isStarted ? '#FFFBEB' : '#F8FAFC';

          return (
            <Box key={item.mode} border="1px solid" borderColor={COLORS.border} bg="white" borderRadius="2xl" p="4">
              <HStack justify="space-between" align="start" gap="2">
                <Box>
                  <Text fontWeight="950" color={COLORS.text}>{item.label}</Text>
                  <Text mt="1" color={COLORS.muted} fontSize="sm">{item.scoreText}</Text>
                </Box>
                <Tag borderRadius="full" bg={statusBg} color={statusColor} flexShrink={0}>{item.status}</Tag>
              </HStack>
...
function Metric({ value, label, className }: { value: string | number; label: string; className?: string }) {
  return (
    <Box className={className} bg="rgba(255,255,255,0.88)" backdropFilter="blur(14px)" border="1px solid" borderColor="#BAE6FD" borderRadius="2xl" p="5" boxShadow="0 12px 30px rgba(31,111,214,0.08)" willChange={className ? 'transform, opacity' : undefined}>
