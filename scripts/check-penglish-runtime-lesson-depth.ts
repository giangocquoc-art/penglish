...
function getLayerStats(depths: LessonDepthAudit[]) {
  ...
  return Array.from(layerKeys).reduce<Record<string, { averageScore: number; weakLessonCount: number; zeroItemLessonCount: number }>>((stats, layerKey) => {
    ...
    stats[layerKey] = {
      ...
      weakLessonCount: layers.filter((layer) => layer.score < 68).length,
  ...
  return {
    ...
    recommendedFixStrategy: lessonValidationSummary.errorCount > 0
      ...
      : lessonValidationSummary.warningCount > 0
        ...
        : shallowLayerLessons.length > 0 || thinLessons.length > 0
          ? 'No blocking runtime errors. Strengthen the weakest lesson layers before release, then verify the learner flow in the browser.'
          : 'All runtime lessons meet the strong-layer threshold. Prioritize browser checks for pacing, clarity, responsive layout, and interaction regressions.',
  };
}
