...
import { Box, SimpleGrid, HStack, VStack, Flex, Text, Button, Icon, Badge, Menu, MenuButton, MenuList, MenuItem } from '@chakra-ui/react';
import { AlertTriangle, BookOpen, HelpCircle, Headphones, PenTool, Target, Zap, Volume2, VolumeX, ChevronDown, CheckCircle2, Home, RotateCcw, Sparkles, Waves } from 'lucide-react';
...
import { getLearningLoopSnapshot, LEARNING_LOOP_UPDATED_EVENT, markLearningLoopCompleted, resolveLearningLoopMistake, type LearningLoopMistakeRecord, type LearningLoopWordRecord } from '../lib/p-english/learning-loop';
...
function LearningLockedScreen({ lessonId, state }: { lessonId: string | null; state: LearningHeartsState }) {
  return (
    <OceanPageShell data-testid="practice-locked-page" variant="vocab" overlayStrength="medium" minH="calc(100vh - 72px)" px="6" py="8" pb={{ base: '28', lg: '8' }}>
      <Box maxW="760px" mx="auto" bg="rgba(255,255,255,0.88)" backdropFilter="blur(16px)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" boxShadow="0 18px 45px rgba(31, 111, 214, 0.10)" data-testid="practice-locked-state">
        <Box px={{ base: '4', md: '6' }} pt="5"><LearningHeartsBadge /></Box>
        <PooSystemState variant="empty" title="Poo đang để bọt biển hồi lại" description={`${OUT_OF_BUBBLES_MESSAGE} Bọt biển sẽ phục hồi sau ${getLockRemainingText(state)}.`} actionLabel="Về trang chủ" actionTo="/home" />
        {lessonId ? (
          <HStack justify="center" px="6" pb="6">
...
function MissingPracticeLessonState({ activityLabel }: { activityLabel: string }) {
  return (
    <OceanPageShell data-testid="practice-missing-lesson-page" variant="vocab" overlayStrength="medium" minH="calc(100vh - 72px)" px={{ base: '4', md: '6' }} py="8" pb={{ base: '28', lg: '8' }}>
      <Box maxW="760px" mx="auto" bg="rgba(255,255,255,0.88)" backdropFilter="blur(16px)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" boxShadow="0 18px 45px rgba(31, 111, 214, 0.10)" data-testid="practice-missing-lesson-state">
        <PooSystemState
          variant="error"
          title={`Poo không tìm thấy bài học để ${activityLabel}`}
          description="Bài học có thể đã đổi đường dẫn hoặc chưa được lưu trên thiết bị này. Bạn mở lộ trình để chọn một bài đang sẵn sàng nhé."
...
export function PracticePage() {
  ...
  const returnTo = searchParams.get('returnTo') === '/today' ? '/today' : '/home';
  ...
  const [reviewedPatterns, setReviewedPatterns] = useState(0);
    ...
    return (
      <OceanPageShell variant="vocab" overlayStrength="medium" minH="calc(100vh - 72px)" px={{ base: '4', md: '6' }} py="8" pb={{ base: '28', lg: '8' }}>
        <Box data-testid="practice-fallback-card" maxW="760px" mx="auto" bg="rgba(255,255,255,0.9)" backdropFilter="blur(16px)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '6', md: '8' }} boxShadow="0 18px 45px rgba(31, 111, 214, 0.10)">
          <Flex justify="center" mb="3" data-testid="practice-fallback-mascot">
  ...
  if (lessonId && mode === 'flashcard') {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="ôn thẻ từ" />;
    }
  ...
  if (lessonId && mode === 'quiz') {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="luyện tập" />;
    }
  ...
  if (lessonId && mode === 'listen') {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="luyện nghe" />;
    }
  ...
  if (lessonId && mode === 'reflex') {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="luyện phản xạ" />;
    }
  ...
  if (lessonId && (mode === 'type' || mode === 'typing')) {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="luyện gõ câu" />;
    }
  ...
  if (lessonId && mode === 'match') {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="ghép cặp" />;
    }
  ...
  if (lessonId && mode === 'speed') {
    if (!lesson) {
      return <MissingPracticeLessonState activityLabel="luyện tốc độ" />;
    }
  ...
  const markActiveUnderstood = () => {
    ...
    setUnderstoodCount((value) => value + 1);
    if (activeReviewItem?.kind === 'weak-word') {
      setReviewedWords((value) => value + 1);
    } else if (activeReviewItem?.kind === 'mistake') {
      setReviewedPatterns((value) => value + 1);
    }
    if (pooReviewIndex >= pooReviewItems.length - 1) {
      markLearningLoopCompleted('practice', 'daily-review', 12);
            ...
            <Box className="penglish-glass-card" bg="rgba(255,255,255,0.72)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '4', md: '7' }} boxShadow="0 16px 38px rgba(31,111,214,0.08)" position="relative" overflow="hidden" data-testid="practice-poo-hero">
              ...
              <Flex position="relative" gap="4" justify="space-between" align={{ base: 'start', md: 'center' }} direction={{ base: 'column', md: 'row' }}>
                <Box minW="0" flex="1">
                  ...
                  <Text as="h1" fontSize={{ base: '2xl', md: '4xl' }} lineHeight="1.06" fontWeight="950" color={PRACTICE_COLORS.text}>Ôn câu Poo nhắc</Text>
              ...
              <Box className="penglish-glass-card" bg="linear-gradient(135deg, rgba(240,253,244,0.92), rgba(232,244,255,0.86))" border="1px solid" borderColor="#BBF7D0" borderRadius="3xl" p={{ base: '4', md: '6' }} data-testid="practice-poo-complete">
                <VStack align="stretch" gap="4">
                  ...
                  <SimpleGrid columns={{ base: 3, md: 3 }} gap="2.5">
                    <Box bg="whiteAlpha.800" border="1px solid" borderColor="#BBF7D0" borderRadius="2xl" p="3"><Text fontWeight="950" color={PRACTICE_COLORS.text}>{understoodCount} câu</Text><Text fontSize="xs" fontWeight="800" color={PRACTICE_COLORS.secondaryText}>đã ôn</Text></Box>
                    <Box bg="whiteAlpha.800" border="1px solid" borderColor="#BAE6FD" borderRadius="2xl" p="3"><Text fontWeight="950" color={PRACTICE_COLORS.text}>{reviewedWords} từ</Text><Text fontSize="xs" fontWeight="800" color={PRACTICE_COLORS.secondaryText}>đã nhớ</Text></Box>
                    <Box bg="whiteAlpha.800" border="1px solid" borderColor="#FED7AA" borderRadius="2xl" p="3"><Text fontWeight="950" color={PRACTICE_COLORS.text}>{reviewedPatterns} mẫu</Text><Text fontSize="xs" fontWeight="800" color={PRACTICE_COLORS.secondaryText}>chắc hơn</Text></Box>
                  </SimpleGrid>
                  <HStack gap="2.5" wrap="wrap">
                    <Button as={Link} to={returnTo} data-testid="practice-poo-back-home" leftIcon={<Icon as={returnTo === '/today' ? Sparkles : Home} />} borderRadius="full" bg={PRACTICE_COLORS.deepBlue} color="white" _hover={{ bg: '#185BB2' }}>{returnTo === '/today' ? 'Tiếp tục phiên hôm nay' : 'Quay về Trang chủ'}</Button>
                ...
                <SimpleGrid columns={{ base: 1, sm: 3 }} gap="3">
                  {[
                    ...
                    <Box key={card.title} bg="rgba(255,255,255,0.82)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '3.5', md: '4' }} minW="0" data-testid="practice-poo-daily-card">
                      ...
                      <Button mt="3" size="sm" minH={{ base: '44px', md: '32px' }} borderRadius="full" variant="outline" onClick={startPooReview}>{card.cta}</Button>
                    </Box>
                  ))}
                </SimpleGrid>
                <SimpleGrid columns={{ base: 1, md: 2 }} gap="3">
                  {pooReviewItems.slice(0, 3).map((item) => (
                    <Box key={item.id} bg="rgba(255,255,255,0.86)" border="1px solid" borderColor={item.isFallback ? '#BAE6FD' : '#FED7AA'} borderRadius="2xl" p="3.5" data-testid="practice-poo-mistake-item">
                      ...
                      <HStack mt="3" gap="2" wrap="wrap"><Button size="xs" minH={{ base: '44px', md: '24px' }} borderRadius="full" onClick={startPooReview}>Tôi hiểu rồi</Button><Button as={Link} to={item.actionPath} size="xs" minH={{ base: '44px', md: '24px' }} borderRadius="full" variant="outline">Mở nguồn</Button></HStack>
                    </Box>
                  ))}
                </SimpleGrid>
              </VStack>
          ...
          <VStack align="stretch" gap="3" minW="0">
            ...
            <Box className="penglish-glass-card" bg="rgba(255,255,255,0.66)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '3.5', md: '5' }} display={{ base: 'none', lg: 'block' }}>
              ...
              <SimpleGrid columns={2} gap="2" mt="3">
                {GAMES.slice(0, 4).map((g) => <Button key={g.id} as={Link} to={`/practice?lessonId=${STARTER_A1_LESSON_ID}&mode=${g.id}`} size="sm" borderRadius="full" variant="outline">{g.name}</Button>)}
              </SimpleGrid>
            </Box>
          </VStack>
        </SimpleGrid>
