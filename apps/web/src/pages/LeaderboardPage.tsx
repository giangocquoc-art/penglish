...
import { Medal, Waves, Zap } from 'lucide-react';
import { get } from '../api';
  ...
  return (
    <Flex justify="center" mb="6">
      ...
      >
        ...
        >
          Chuỗi học <Icon as={Waves} boxSize="4" ml="1" aria-hidden="true" />
        </Button>
      </ButtonGroup>
    </Flex>
  );
      ...
      <Flex align="center" justify="space-between" gap="4" wrap="wrap">
        <HStack gap="4">
          ...
          <VStack align="start" gap="0">
            ...
            <Text fontSize="sm" color="gray.700" fontWeight="600">
              {boardEmpty ? 'Chưa có dữ liệu' : notInBoard ? 'Bạn chưa tham gia' : `${metric} ${metricLabel}`}
            </Text>
          </VStack>
        </HStack>
        <VStack align="end" gap="0">
          ...
          <Text fontSize="xs" color="gray.500" mt="1">
            {boardEmpty ? 'Chưa có xếp hạng' : notInBoard ? 'Bạn chưa có hạng' : 'Hạng của bạn'}
          </Text>
        </VStack>
      </Flex>
    </MotionBox>
...
function TopTable({ entries, mode }: { entries: Entry[]; mode: Mode }) {
  ...
  return (
    <Box bg="white" borderRadius="2xl" boxShadow="card" overflow="hidden">
      ...
      {entries.map((e, i) => {
        const overall = i + 1;
        const medalColor = overall === 1 ? '#D97706' : overall === 2 ? '#64748B' : overall === 3 ? '#B45309' : null;
        ...
        return (
          ...
          >
            <Box w="60px">
              {medalColor ? (
                <Flex w="32px" h="32px" align="center" justify="center" aria-label={`Hạng ${overall}`}>
                  <Icon as={Medal} boxSize="7" color={medalColor} strokeWidth="2.3" aria-hidden="true" />
                </Flex>
              ) : (
                <Flex
                  w="32px"
            ...
            <HStack w="80px" justify="flex-end" gap="1">
              ...
              {mode === 'streak' ? (
                ...
                <Icon as={Zap} boxSize="4" color="#D97706" fill="#FEF3C7" aria-hidden="true" />
              )}
            </HStack>
          </MotionHStack>
        );
...
export function LeaderboardPage() {
  ...
  const notInBoard = me != null && status === 'ready' && entries.length > 0 && myRank == null;
  const boardEmpty = me != null && status === 'ready' && entries.length === 0;
  ...
  return (
    <Box px="6" pb="10" maxW="900px" mx="auto">
      ...
      {status === 'loading' ? (
        <PooSystemState variant="loading" title="Poo đang chuẩn bị bảng xếp hạng..." description="Poo đang đếm từng nhịp học để xếp đúng vị trí." />
      ) : status === 'error' ? (
        <PooSystemState variant="error" title="Poo chưa tải được bảng xếp hạng" description="Dữ liệu thành tích chưa về kịp. Bạn tải lại để Poo đếm lại nhé." actionLabel="Tải lại bảng xếp hạng" onAction={() => setRetryKey((key) => key + 1)} />
      ) : (
        <>
          {me && (
            <SelfRankCard
