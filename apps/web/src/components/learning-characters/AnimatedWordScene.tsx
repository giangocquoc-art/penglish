...
const SIZE_STYLES: Record<WordSceneSize, { minH: string; visualW: string; pad: string; compact: boolean }> = {
  compact: { minH: '138px', visualW: '132px', pad: '3', compact: true },
  normal: { minH: '176px', visualW: '168px', pad: '4', compact: false },
  large: { minH: '218px', visualW: '210px', pad: '5', compact: false },
};

const WORD_VISUAL_ICONS: Record<WordVisualIcon, LucideIcon> = {
  activity: Activity,
  apple: Apple,
  backpack: Backpack,
  badge: ContactRound,
...
function SceneIcon({ name, color, boxSize = '6' }: { name: WordVisualIcon; color: string; boxSize?: string }) {
  return <Icon as={WORD_VISUAL_ICONS[name]} color={color} boxSize={boxSize} strokeWidth="2.4" aria-hidden="true" />;
}
...
function WhaleCharacter({ waving = false }: { waving?: boolean }) {
  return (
    <Box className="word-scene-character" position="relative" w="104px" h="82px" aria-hidden="true" willChange="transform, opacity">
      <OceanMascot mascot="poo" pose="idle" size="sm" decorative motion="float" position="absolute" left="4px" bottom="0" />
      {waving ? (
        <Icon as={Hand} position="absolute" right="0" top="2px" boxSize="7" color={SCENE_COLORS.deepBlue} strokeWidth="2.4" transform="rotate(10deg)" />
      ) : null}
    </Box>
  );
}
...
function Person({ label, color = '#DBEAFE' }: { label?: string; color?: string }) {
  return (
    <VStack gap="1" aria-hidden="true">
      <Flex w="46px" h="46px" borderRadius="full" bg={color} align="center" justify="center">
        <Icon as={UserRound} boxSize="6" color={SCENE_COLORS.deepBlue} strokeWidth="2.3" />
      </Flex>
      {label ? (
  ...
  if (key === 'good morning') {
    return (
      <Box className="word-scene-visual" position="relative" w="118px" h="104px" aria-hidden="true" willChange="transform, opacity">
        ...
        <Icon as={Sun} position="absolute" right="5px" top="8px" boxSize="6" color="#B45309" strokeWidth="2.4" />
      </Box>
    );
  }
  ...
  if (key === 'nice to meet you') {
    return (
      <HStack className="word-scene-visual" gap="3" align="center" aria-hidden="true" willChange="transform, opacity">
        ...
        <Icon as={Handshake} boxSize="7" color={scene.accentColor} strokeWidth="2.3" />
  ...
  if (key === 'thank you') {
    return (
      <VStack className="word-scene-visual" gap="2" aria-hidden="true" willChange="transform, opacity">
        <Icon as={Flower2} boxSize="14" color={scene.accentColor} strokeWidth="2" />
  ...
  if (scene.category === 'school' || scene.category === 'classroom') {
    return (
      <VStack className="word-scene-visual" gap="2" aria-hidden="true" willChange="transform, opacity">
        ...
        <HStack gap="3">
          <Icon as={Pencil} boxSize="7" color={scene.accentColor} strokeWidth="2.3" />
          <Icon as={Backpack} boxSize="7" color={scene.accentColor} strokeWidth="2.3" />
        </HStack>
      </VStack>
    );
  }
    ...
    >
      ...
      {scene.category === 'food' ? (
        <Box {...VISUAL_SCENE_BASE}>
          ...
          <Box position="absolute" left="44px" top="22px"><SceneIcon name={scene.icon} color={scene.accentColor} boxSize="8" /></Box>
      ...
      {scene.category === 'emotion' ? (
        <Box {...VISUAL_SCENE_BASE}>
          ...
          <Icon className="word-scene-motion" as={Heart} position="absolute" right="19px" top="18px" color={scene.accentColor} boxSize="6" fill={scene.softColor} strokeWidth="2.4" />
      ...
      <Flex position="relative" direction={{ base: 'column', md: 'row' }} align="center" justify="space-between" gap={size === 'compact' ? '3' : '5'}>
        ...
        <VStack align="start" gap={size === 'compact' ? '2' : '3'} flex="1" w="100%">
          <HStack className="word-scene-detail" wrap="wrap" gap="2" willChange="transform, opacity">
            <HStack as="span" px="2.5" py="1" borderRadius="full" bg={scene.softColor} color={scene.accentColor} fontSize="xs" fontWeight="950" gap="1.5">
              <SceneIcon name={scene.icon} color={scene.accentColor} boxSize="3.5" />
              <Text as="span">{scene.title}</Text>
            </HStack>
