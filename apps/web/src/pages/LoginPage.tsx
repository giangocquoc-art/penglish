...
export function LoginCallbackPage() {
  ...
  return (
    <>
      <AuthLoadingScreen />
      <AuthCallbackContent params={params} />
    </>
  );
}
...
function AuthCallbackContent({ params }: { params: URLSearchParams }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const storedRedirect = window.sessionStorage.getItem('penglish-auth-redirect');
        intended = storedRedirect ? sanitizeAuthRedirect(storedRedirect) : fallback;
      ...
      try {
        ...
        const remainingLoadingTime = Math.max(
          0,
          minimumLoadingDuration - (window.performance.now() - loadingStartedAt),
        );
        window.setTimeout(() => redirectToLearning(intended), remainingLoadingTime);
      }
    };
