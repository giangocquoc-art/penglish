...
export function LessonMatchPractice({ lesson, onWhaleMoodChange }: { lesson: EnglishLesson; onWhaleMoodChange?: (mood: WhaleMood) => void }) {
  ...
  const totalAttempts = completedPairIds.length + mistakes;
  const percentage = totalAttempts > 0 ? Math.round((completedPairIds.length / totalAttempts) * 100) : 0;
    ...
    return (
      <Box bg={COLORS.bg} minH="calc(100vh - 72px)" px="6" py="8">
        <Box maxW="760px" mx="auto" bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="3xl">
          <PooSystemState variant="empty" title="Poo chưa có cặp từ để ghép" description="Bài này đang ưu tiên nhịp luyện khác. Bạn có thể mở sổ từ để chọn một nhóm phù hợp hơn." actionLabel="Mở sổ từ vựng" actionTo="/vocabularies" />
        </Box>
      </Box>
    );
  }
          ...
          <Box bg="rgba(255,255,255,0.92)" backdropFilter="blur(16px)" border="1px solid" borderColor="#BAE6FD" borderRadius="3xl" p={{ base: '5', md: '7' }} boxShadow="0 16px 38px rgba(31, 111, 214, 0.08)">
            <Flex justify="space-between" align={{ base: 'start', md: 'center' }} direction={{ base: 'column', md: 'row' }} gap="4">
              <Box>
                ...
                <Text color={COLORS.muted}>Ưu tiên nhận diện từ vựng nghĩa tiếng Việt trước, không dịch từng chữ.</Text>
              </Box>
