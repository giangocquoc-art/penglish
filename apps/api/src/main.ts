...
type ShopItem = {
  ...
  price: number;
  description: string;
  type: 'streak_freeze' | 'theme' | 'sound' | 'avatar' | 'background' | 'special';
  visualKey: 'streak-shield' | 'theme-blue' | 'typing-sfx' | string;
  imageUrl?: string | null;
};

const DEFAULT_SHOP_ITEMS: ShopItem[] = [
  { id: 'item-streak-shield', name: 'Streak Shield', price: 150, description: 'Giữ streak 1 ngày', type: 'streak_freeze', visualKey: 'streak-shield' },
  { id: 'item-theme-blue', name: 'Blue Theme', price: 80, description: 'Giao diện xanh', type: 'theme', visualKey: 'theme-blue' },
  { id: 'item-typing-sfx', name: 'Typing SFX', price: 60, description: 'Âm thanh gõ phím', type: 'sound', visualKey: 'typing-sfx' },
];
...
function defaultState(): AppState {
  return {
    ...
    folders: {
      newest: [
        { id: 1001, name: 'Starter Pack', icon: 'folder', isShared: true, upvoteCount: 0, createdAt: new Date().toISOString(), categoryCount: 3, wordCount: 156, user: { id: 1, name: 'Demo User' } },
        { id: 1002, name: 'IELTS Reading', icon: 'book-open', isShared: true, upvoteCount: 0, createdAt: new Date().toISOString(), categoryCount: 4, wordCount: 343, user: { id: 2, name: 'Demo User 2' } },
      ],
      trending: [
        { id: 2001, name: 'PTE Pack', icon: 'flame', isShared: true, upvoteCount: 27, createdAt: new Date().toISOString(), categoryCount: 4, wordCount: 4986, user: { id: 3, name: 'Demo User 3' } },
        { id: 2002, name: 'VOCAB IN USE', icon: 'library', isShared: true, upvoteCount: 15, createdAt: new Date().toISOString(), categoryCount: 11, wordCount: 358, user: { id: 4, name: 'Demo User 4' } },
      ],
      mine: [
        { id: 3001, name: 'Folder cá nhân', icon: 'folder-personal', isShared: false, upvoteCount: 0, createdAt: new Date().toISOString(), categoryCount: 2, wordCount: 80, user: { id: 'demo-user', name: 'Mr Minh' } },
      ],
    },
    ...
    shop: {
      coins: 120,
      items: structuredClone(DEFAULT_SHOP_ITEMS),
...
function normalizeState() {
  ...
  const folderIconAliases: Record<string, string> = {
    '\u{1F4C1}': 'folder',
    '\u{1F4D8}': 'book-open',
    '\u{1F525}': 'flame',
    '\u{1F4DA}': 'library',
    '\u{1F5C2}\uFE0F': 'folder-personal',
  };
  for (const bucket of [state.folders.newest, state.folders.trending, state.folders.mine]) {
    for (const folder of bucket) folder.icon = folderIconAliases[folder.icon] ?? folder.icon ?? 'folder';
  }
  const persistedShop = state.shop;
  const defaultShopById = new Map(DEFAULT_SHOP_ITEMS.map((item) => [item.id, item]));
  state.shop = {
    coins: Number(persistedShop?.coins ?? 120),
    items: Array.isArray(persistedShop?.items)
      ? persistedShop.items.map((item) => {
          const fallback = defaultShopById.get(String(item.id));
          return {
            id: String(item.id),
            name: String(item.name ?? fallback?.name ?? 'Vật phẩm PooEnglish'),
            price: Number(item.price ?? fallback?.price ?? 0),
            description: String(item.description ?? fallback?.description ?? ''),
            type: item.type ?? fallback?.type ?? 'special',
            visualKey: item.visualKey ?? fallback?.visualKey ?? 'streak-shield',
            imageUrl: item.imageUrl ?? fallback?.imageUrl ?? null,
          };
        })
      : structuredClone(DEFAULT_SHOP_ITEMS),
    inventory: Array.isArray(persistedShop?.inventory) ? persistedShop.inventory.map(String) : [],
    equippedSound: persistedShop?.equippedSound == null ? null : String(persistedShop.equippedSound),
    equippedTypingSound: persistedShop?.equippedTypingSound == null ? null : String(persistedShop.equippedTypingSound),
    customUploadPrice: Number(persistedShop?.customUploadPrice ?? 30),
  };
...
function isDocumentNavigation(req: Request) {
  return req.headers['sec-fetch-dest'] === 'document' || String(req.headers.accept ?? '').includes('text/html');
}
...
app.get('/categories', (req, res, next) => {
  if (isDocumentNavigation(req)) return next();
  const user = authenticatedUser(req);
  res.json({ data: state.categories.filter((category) => category.ispublic || (user ? categoryOwnedBy(category, user) : false)) });
});
...
app.get('/vocabularies', (req, res, next) => {
  if (isDocumentNavigation(req)) return next();
  const user = authenticatedUser(req);
  res.json({ data: user ? state.vocabularies.filter((item) => vocabOwnedBy(item, user)) : [] });
});
...
app.get('/shop/coins', (req, res) => {
  const user = authenticatedUser(req);
  res.json({ coins: user ? shopForUser(user).coins : 0 });
});
app.get('/shop/inventory', (req, res) => {
  const user = authenticatedUser(req);
  res.json({ data: user ? shopForUser(user).inventory : [] });
});
...
app.get('/shared-streak', (req, res, next) => {
  if (isDocumentNavigation(req)) return next();
  const user = authenticatedUser(req);
  res.json({ data: user ? sharedStreakForUser(user) : { inviter: '', invitee: null, status: '', invitations: [] } });
});
...
app.get('/folders', (req, res, next) => {
  if (isDocumentNavigation(req)) return next();
  const user = authenticatedUser(req);
  res.json({ data: user ? state.folders.mine.filter((folder) => folderOwnedBy(folder, user)) : [] });
});
app.post('/folders', (req, res) => {
  const user = requireAuth(req, res);
  if (!user) return;
  const folder: Folder = { id: Date.now(), name: String(req.body?.name ?? 'Folder'), icon: String(req.body?.icon ?? 'folder'), isShared: false, upvoteCount: 0, createdAt: new Date().toISOString(), categoryCount: 0, wordCount: 0, user: { id: user.id, name: user.name } };
...
app.get('/api/subscriptions', (req, res) => {
  const user = authenticatedUser(req);
  res.json({ data: user ? state.subscriptions.filter((item) => sameId(item.userId, user.id)) : [] });
});
...
app.get('/ai/usage', (req, res) => {
  const user = authenticatedUser(req);
  if (!user) return res.json({ data: { usage: 0 } });
...
app.post('/ai/generate-vocabulary', (req, res) => {
  ...
  const usage = user ? incrementAiUsage(user) : 0;
  const terms = text
