    ...
    return (
      <Box bg={COLORS.bg} minH="calc(100vh - 72px)" px="6" py="8">
        <Box maxW="760px" mx="auto" bg="white" border="1px solid" borderColor={COLORS.border} borderRadius="3xl">
          <PooSystemState variant="empty" title="Poo chưa tạo được lượt luyện tốc độ" description="Bài này chưa có đủ từ vựng, câu thử sức hoặc phản xạ để tạo một lượt công bằng." actionLabel="Quay về bài học" actionTo={`/lessons/${lesson.id}`} />
        </Box>
      </Box>
    );
  }
