import { Box, HStack, Icon, Image, Text, Tooltip, VStack } from '@chakra-ui/react';
...
export function BrandLogo({ variant = 'compact', size = 'md', showSubtitle = false }: BrandLogoProps) {
  ...
  const tooltip = LOGO_TOOLTIPS[(variant.length + size.length) % LOGO_TOOLTIPS.length];
    ...
    <Tooltip label={tooltip} hasArrow borderRadius="full" bg="#102A43" color="white" px="3" py="2" openDelay={180} shouldWrapChildren>
      ...
      >
          <Image
            ...
            fallback={<Icon as={BookOpen} boxSize={size === 'sm' ? '5' : '6'} color="#1F6FD6" aria-hidden="true" />}
          />
        </Box>
